...
update_agent_file() {
    ...
    if [ ! -f "$target_file" ]; then
        ...
        if [ -f "$REPO_ROOT/.claude/.sdd/templates/agent-file-template.md" ]; then
            cp "$REPO_ROOT/.claude/.sdd/templates/agent-file-template.md" "$temp_file"
        else
            echo "ERROR: Template not found at $REPO_ROOT/.claude/.sdd/templates/agent-file-template.md"
            return 1
        fi
